...
W tym rozdziale omówiono techniki analizy danych, które są wykorzystywane w wielu dziedzinach nauki i przemysłu. Szczegółowo przedstawiono klasteryzację, przetwarzanie języka naturalnego, wektoryzację i metody redukcji wymiarowości.
...
\section{Klasteryzacja}
	...
	\subsection{Metody klasteryzacji}
		...
		\begin{enumerate}
			...
			\item \textbf{Algorytmy oparte na gęstości danych}. Te algorytmy obejmują różne techniki. Pierwszym z nich jest \textbf{K-średnich (K-means)}, który jest iteracyjnym algorytmem opartym na minimalizacji sumy kwadratowych odchyleń punktów klastrów od ich centrów. Następnie mamy \textbf{Rozpowszechnianie podobieństwa (Affinity Propagation)}, algorytm, który rozprzestrzenia wiadomości o podobieństwie między parami obiektów w celu wyboru typowych przedstawicieli każdego klastra. Kolejną metodą jest \textbf{Przesunięcie średniej (mean shift)}, wybierająca centroidy klastrów w obszarach o największej gęstości. \textbf{Klasteryzacja spektralna (spectral clustering)} to metoda wykorzystująca wartości własne macierzy odległości do zmniejszenia wymiarowości przed zastosowaniem innych metod klasteryzacji. W końcu mamy \textbf{DBSCAN (Density-Based Spatial Clustering of Applications with Noise)}, algorytm grupujący punkty w jeden klaster w obszarach o wysokiej gęstości i oznaczający odosobnione punkty jako szum.
		\end{enumerate}

	\subsection{Etapy klasteryzacji}
		Niezależnie od przedmiotu badań, zastosowanie analizy klastrów obejmuje następujące etapy. Pierwszym z nich jest wybór próby do klasteryzacji. Zakłada się, że klasyfikować można tylko dane ilościowe, ponieważ wskaźniki liczbowe mogą być ilościowo oceniane. Następnie określa się zestaw zmiennych, na podstawie których obiekty w próbie będą oceniane, tj. przestrzeni cech. Cechy mogą być różnymi charakterystykami obiektów, które mają znaczenie dla badań. Kolejnym etapem jest obliczanie wartości wybranej miary podobieństwa (lub różnicy) między obiektami. Celem jest określenie, w jakim stopniu obiekty w próbie są do siebie podobne lub różnią się względem wybranych cech. Następnie stosuje się metodę analizy klastrów do stworzenia grup podobnych obiektów, zakładając wykorzystanie jednego z istniejących algorytmów klasteryzacji w taki sposób, aby obiekty w jednym klastrze były maksymalnie podobne do siebie, a obiekty z różnych klastrów maksymalnie się różniły. Ostatnim etapem jest sprawdzenie wiarygodności wyników klasteryzacji, co może być osiągnięte za pomocą różnych metod, takich jak wizualizacja klastrów, analiza stabilności klastrów i ocena jakości podziału.

	\subsection{Zastosowanie}
		Zastosowanie metody analizy klastrów jest z powodzeniem wykorzystywane w wielu dziedzinach i dyscyplinach. Metoda ta jest również szeroko stosowana w informatyce. Najbardziej udanymi przykładami wykorzystania klasteryzacji w pracy z danymi tekstowymi są: \textbf{klasteryzacja wyników wyszukiwania}, która polega na grupowaniu dokumentów zwróconych w odpowiedzi na zapytanie użytkownika, aby podobne dokumenty pojawiały się razem \cite{SearchClustering};\textbf{ grupowanie użytkowników o podobnych zainteresowaniach}, co pozwala na lepszą personalizację treści, rekomendacje oraz targetowanie w reklamie \cite{UserClustering}; \textbf{grupowanie tekstowych dokumentów}, takich jak artykuły prasowe, publikacje naukowe czy wiadomości w mediach społecznościowych, które polega na organizowaniu ich według tematyki, co ułatwia użytkownikom odnalezienie interesujących ich treści \cite{SearchClustering}.
...
\section{Przetwarzanie języka naturalnego}
	...
	\begin{itemize}
		...
		\item wektoryzacja.
	\end{itemize}
	W kolejnych podrozdziałach szczegółowo omówiono trzy ostatnie metody, ponieważ są one bardziej skomplikowane pod względem koncepcji oraz implementacji.


	\subsection{Stemming}
		 Stemming to proces redukcji słowa do jego podstawowej formy, zwanej rdzeniem lub stemem. Liczne formy wyrazowe różniące się sufiksami, przedrostkami, końcówkami itp. utrudniają tworzenie słowników i dalsze przetwarzanie. Stemming sprowadza słowo do jego podstawowej formy. Istota podejścia polega na znalezieniu rdzenia słowa, w tym celu z końca i początku słowa kolejno odcinane są jego części zgodnie z ustalonymi regułami. Wadą podejścia jest możliwa utrata informacji podczas odcinania części, przez co możemy stracić informację o części mowy.
		 
		 

	\subsection{Lematyzacja}		
		Podejście to stanowi alternatywę dla stemmingu. Główna idea polega na sprowadzeniu słowa do formy słownikowej -- lematu. Przykładem lematyzacji dla języka polskiego jest sprowadzenie rzeczowników do mianownika liczby pojedynczej, przymiotników do mianownika liczby pojedynczej rodzaju męskiego, a czasowników, imiesłowów i rzeczowników odczasownikowych do formy bezokolicznika aspektu niedokonanego.
...
\section{Wektoryzacja}
	...
	\subsection{Cele wektoryzacji}		
		Główne cele wektoryzacji tekstu obejmują reprezentowanie tekstu do analizy przez algorytmy maszynowe, znajdowanie podobieństw między tekstami lub kategoryzowanie tekstów według tematyki, rozwiązywanie zadań klasyfikacji, klasteryzacji lub regresji na podstawie danych tekstowych oraz tworzenie modeli uczenia maszynowego do automatycznego uogólniania informacji tekstowych.
	...
	\subsection{Zastosowanie}		
		Wektoryzacja tekstu znajduje zastosowanie w wielu dziedzinach, w tym w analizie nastrojów i sentymentów w mediach społecznościowych i recenzjach, klasyfikacji tekstów według tematyki lub kategorii, systemach rekomendacyjnych opartych na opisach tekstowych oraz automatycznym tłumaczeniu tekstu i implementacji chatbotów.
